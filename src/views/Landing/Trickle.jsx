...
const Trickle = ({ account, curpage, setCurPage }) => {
    ...
    return (
        <>
            ...
            <ShardHolders active={curpage === 3}>
                <Box width={'455px'} position={'relative'} onClick={() => sm && (curpage === 3 ? setCurPage(0) : setCurPage(3))} >
                    <Box fontSize={xs ? '24px' : '36px'} lineHeight={xs ? '41px' : '55px'} fontWeight={'700'} textAlign={xs && curpage !== 3 ? 'center' : 'left'}>Trickle Vault</Box>
                    <Box pt={xs ? '7px' : '18px'} lineHeight={xs ? '14px' : '19px'} fontSize={xs ? '12px' : '16px'}>
                        <Box>
                            Welcome to our Triclke Vault. When you deposit your tokens into this vault you forfit them to the contract. In return you have 365 claims.
                            <br /><br />
                            The claim level is tiered from basic to <br />platinum depending on which nft you hold. This is determined below when you connect your wallet.
                            <br /><br />
                            Once you reach your max claim allowance you will have o start a new wallet to avail of the rewards from this vault.
                            <br /><br />
                            Please note that this is a high reward compounding vault and best practise here is to compound daily.
                ...
                <Box width={'503px'}>
                    ...
                    <InfoPanel border={'2px solid #FFFFFF'} mt={xs ? '14px' : '10px'}>
                        ...
                        <Box fontWeight={600} fontSize={xs ? '15px' : '20px'}>xxx</Box>
                    </InfoPanel>
                    <Box mt={xs ? '13px' : '20px'} display={'flex'} justifyContent={'space-between'} >
                        <StyledButton width={xs ? '98px' : '124px'} height={'33px'}>
                            ENTER VAULT
                        </StyledButton>

                    </Box >
                    <Box mt={xs ? '21px' : '55px'} borderTop={'2px solid #FFFFFF'} pt={'30px'}>
                ...
                <Box width={'397px'}>
                    ...
                    <Box mt={xs ? '14px' : '25px'} width={'100%'} height={'2px'} bgcolor={'white'} />
                    <Box display={'flex'} justifyContent={'space-between'} mt={'20px'} >
                        <Box lineHeight={'13px'}>MY TOKENS</Box>
                        <Box fontSize={xs ? '24px' : '36px'} fontWeight={500} lineHeight={xs ? '18px' : '24px'} mt={xs ? 0 : '5px'}>XXXX</Box>
                    </Box>
                    <Box mt={xs ? '26px' : '32px'} width={'100%'} height={'2px'} bgcolor={'white'} />
                    <Box display={'flex'} justifyContent={'space-between'} mt={'20px'} >
                        <Box lineHeight={'13px'}>TOTAL REW.</Box>
                        <Box fontSize={xs ? '24px' : '36px'} fontWeight={500} lineHeight={xs ? '18px' : '24px'} mt={xs ? 0 : '5px'}>XXXX</Box>
                    </Box>
                    <Box mt={xs ? '26px' : '32px'} width={'100%'} height={'2px'} bgcolor={'white'} />
                    <Box display={'flex'} justifyContent={'space-between'} mt={'24px'} color={'#00D8BD'}>
                        <Box lineHeight={'13px'}>VAULT TOTAL</Box>
                        <Box fontSize={xs ? '24px' : '36px'} fontWeight={500} lineHeight={xs ? '18px' : '24px'} mt={xs ? 0 : '5px'}>XXXX</Box>
                    </Box>
                    <Box mt={xs ? '21px' : '37px'} borderTop={!sm ? 'none' : '2px solid #FFFFFF'} pt={!sm ? '0' : '30px'}>
                        <Box display={'flex'} justifyContent={'space-between'} alignItems={xs ? 'end' : 'center'} flexDirection={xs ? 'column-reverse' : 'row'}>
                            <StyledButton type={'secondary'} width={xs ? '135px' : '184px'} height={'33px'} >
                                CLAIM
                            </StyledButton>
